...
function boot() {
    ...
    // if shutdown() was previously called explicitly, don't display
    // the terminal until the user clicks reboot():
    let shutdownState = localStorage.getItem("shutdown");
    if (shutdownState) {
        return shutdown();
    } 

    // otherwise, now that everything is initialized, hide the static data
    // and display the TUI:
...
// reboot: click handler for link displayed by shutdown() to restart TUI
function reboot(args) {
    localStorage.removeItem("shutdown");

    // clear reboot message left by shutdown():
    let termRoot = document.getElementById("terminal"); 
    termRoot.innerHTML = ""; 
...
function shutdown(args) {
    ...
    // make sure #ui and thus termRoot is visible, in case boot()
    // didn't already do this after a previous shutdown() on another
    // page, but don't let it take up the whole screen:
    let uiRoot = document.getElementById("ui");
    uiRoot.style.display = "flex"; 
    uiRoot.style.height = ""; 
    ...
    // store shutdown state in localStorage so that if someone calls shutdown()
    // on one page, they don't land back in the terminal interface on a new page
...
function sh(input) {
    ...
    if (cmdStruct) {
        cmdStruct.impl(args);
    } else if (document.getElementById(args[0])) {
    ...
    // TODO: catch all errors here so the command loop doesn't break...
...
const COMMANDS = {
    ...
    nav: { impl: nav, help: `: list site navigation links`, aliases: []},
    'goto': { impl: cn, help: ` NAME: go to NAME (NAME alone is a shortcut)`, aliases: ['go', 'cd', 'jump']},
    ...
    clear: { impl: cls, help: `: clear terminal and display`, aliases: ['cls'] },
    bling: { impl: bling, help: ` N: change to color scheme N`, aliases: [] },
    shutdown: { impl: shutdown, help: `: turn off this interface`, aliases: ['quit', 'exit', 'die']},
    // supported, but not show in help:
    ...
    who: { impl: who, help: `: who am I?`, aliases: ['w', 'finger'], hidden: true},
};
...
function bling(args) {
    ...
    if (schemeIdx < colors.length) {
        ...
        // TODO: store newBg and newFg in localStorage for persistence
...
function nav(args) {
    ...
    let navRoot = document.getElementById("nav");
    let navTOC = tocUnder(navRoot);

    printTOC(navTOC, 0);
    printLine("\nType the name of a page to visit it.")
}
...
function getDesc(node) {
    ...
    case "A": {
        ...
        if (node.textContent.length < 50) {
            return node.textContent.trim().replaceAll("\n", " ") + " (link)";
        }
    ...
    case "IMG": {
        ...
        } else if (node.alt && node.alt !== "") {
            return (node.alt).replaceAll("\n", " ") + " (image)";
        } else {
            return "(image)";
        }
    }
    case "FIGURE": {
        for (c of node.children) {
            if (c.tagName === "FIGCAPTION") {
                return c.innerText.replaceAll("\n", " ") + " (image)";
